...
func (s *SmartContract) Owner_register(ctx contractapi.TransactionContextInterface, owner_id string, bot_types string, costs uint) (*rsa.PublicKey, error) {
...
func (s *SmartContract) Renter_register(ctx contractapi.TransactionContextInterface, renter_id string, business_type string) (*rsa.PublicKey, error) {
...
func (s *SmartContract) Add_vehicle(ctx contractapi.TransactionContextInterface, owner_veh_id string, vehicle_id string, type_of_work string) (*rsa.PublicKey, error) {
...
func (s *SmartContract) Create_order(ctx contractapi.TransactionContextInterface, order_properties string, renter_id string) error {
	exists, err := s.ThisExists(ctx, renter_id)
	if err != nil {
		return err
	}
	if exists {
		return fmt.Errorf("the renter %s already exists", renter_id)
	}
...
func (s *SmartContract) UpdateOwner(ctx contractapi.TransactionContextInterface, owner_id string, bot_types string, costs uint) error {
	exists, err := s.ThisExists(ctx, owner_id)
	if err != nil {
		return err
	}
...
// UpdateOwner updates an existing user in the world state with provided parameters.
func (s *SmartContract) UpdateRenter(ctx contractapi.TransactionContextInterface, renter_id string, business_type string) error {
	exists, err := s.ThisExists(ctx, renter_id)
	if err != nil {
		return err
	}
...
func (s *SmartContract) UpdateVehicle(ctx contractapi.TransactionContextInterface, owner_veh_id string, vehicle_id string, type_of_work string) error {
	exists, err := s.ThisExists(ctx, vehicle_id)
	if err != nil {
		return err
	}
